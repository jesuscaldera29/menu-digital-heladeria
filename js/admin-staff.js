  ...
  container.innerHTML = allStaff.map(s => `
    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 flex-wrap gap-3">
      ...
      <div class="flex gap-2">
        <button onclick="clockIn('${s.id}')" class="bg-green-100 text-green-700 px-3 py-2 rounded-xl text-xs font-bold active:scale-95 transition-all">▶ Entrada</button>
        <button onclick="clockOut('${s.id}')" class="bg-red-100 text-red-700 px-3 py-2 rounded-xl text-xs font-bold active:scale-95 transition-all">⏹ Salida</button>
...
async function clockOut(staffId) {
  try {
    ...
    if (!data?.length) return showToast('⚠️ No hay entrada activa hoy', 'error');
    const shift = data[0];
    const clockOut = new Date();
    const hours = ((clockOut - new Date(shift.clock_in)) / 3600000).toFixed(2);
    const { error } = await supabaseClient.from('shifts').update({ clock_out: clockOut.toISOString(), total_hours: parseFloat(hours) }).eq('id', shift.id);
    if (error) throw error;
...
async function loadShifts() {
  try {
    const { data } = await supabaseClient.from('shifts').select('*, staff(name)').eq('business_id', businessId).order('date', { ascending: false }).limit(30);
    ...
    tbody.innerHTML = data.map(s => `<tr>
      ...
      <td class="text-sm">${s.clock_out ? new Date(s.clock_out).toLocaleTimeString() : '⏳ Activo'}</td>
