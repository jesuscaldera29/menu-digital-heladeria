...
async function bridgePrintTicket(order, settings) {
  ...
  const data = {
    ...
    delivery_method: ((order.delivery_method || order.delivery_type || 'Local') + ' - ' + (order.customer_name && order.customer_name !== 'VENTA RAPIDA' ? order.customer_name : 'Mostrador')).toUpperCase(),
